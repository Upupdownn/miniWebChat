...
MySQLConnectionPool::PooledConnection::PooledConnection(
...
MySQLConnectionPool::PooledConnection::PooledConnection(PooledConnection&& other) noexcept
...
MySQLConnectionPool::PooledConnection::operator=(PooledConnection&& other) noexcept
{
    if (this != &other)
    {
        release();
...
MySQLConnection* MySQLConnectionPool::PooledConnection::operator->() const
{
    return conn_;
}
...
MySQLConnection* MySQLConnectionPool::PooledConnection::get() const
{
    return conn_;
}

bool MySQLConnectionPool::PooledConnection::valid() const
{
    return conn_ != nullptr;
}

void MySQLConnectionPool::PooledConnection::release()
{
...
bool MySQLConnectionPool::init()
{
    ...
    for (std::size_t i = 0; i < config_.min_connections; ++i)
    {
        ...
        if (!conn->connect(config_.db_config))
        {
            return false;
        }

        MySQLConnection* raw = conn.get();
...
MySQLConnectionPool::PooledConnection MySQLConnectionPool::acquire()
{
    std::unique_lock<std::mutex> lock(mutex_);

    if (!initialized_)
    {
        throw std::runtime_error("MySQLConnectionPool is not initialized");
    }
    ...
    while (true)
    {
        if (shutting_down_)
        {
            throw std::runtime_error("MySQLConnectionPool is shutting down");
        }

        if (!idle_connections_.empty())
        {
            MySQLConnection* conn = idle_connections_.front();
            idle_connections_.pop();
...
std::size_t MySQLConnectionPool::totalConnectionCount() const
{
    std::lock_guard<std::mutex> lock(mutex_);
    return all_connections_.size();
}

std::size_t MySQLConnectionPool::idleConnectionCount() const
{
    std::lock_guard<std::mutex> lock(mutex_);
